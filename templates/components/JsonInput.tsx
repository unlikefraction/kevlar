import { JsonInput as MantineJsonInput, type JsonInputProps as MantineJsonInputProps } from '@mantine/core';
import {
  baseInputStates,
  baseInputActions,
  baseInputInput,
...
// JsonInput inherits from Textarea pattern.
// keyboard.bindings.Enter: insertNewline (NOT submit).
// keyboard.bindings['Ctrl+Enter']: submit.
// Dev-fill: onFormatOnBlur, onValidate.
...
const input      = {
  ...
  keyboard: {
    ...
    bindings: {
      ...
      Enter:          (ctx: KevlarContext) => { ctx.insertNewline(); },
      'Ctrl+Enter':   (ctx: KevlarContext) => { ctx.submitForm(); },
    },
  },
};
...
const validation = {
  ...baseInputValidation,
  onFormatOnBlur: FUNCTION_MUST_BE_DEFINED as unknown as (value: string) => string,
  onValidate:     FUNCTION_MUST_BE_DEFINED as unknown as (value: string) => string | null,
};


// ─── PROPS ─────────────────────────────────────────────────────

export type KevlarJsonInputProps = MantineJsonInputProps & {
  // REQUIRED
  ...
  // Dev-fill: JSON-specific
  onFormatOnBlur?: (value: string) => string;
...
export function JsonInput(props: KevlarJsonInputProps) {
  ...
  if (onFormatOnBlur) spec.validation.onFormatOnBlur = onFormatOnBlur;
  if (onValidate) spec.validation.onValidate = onValidate;
